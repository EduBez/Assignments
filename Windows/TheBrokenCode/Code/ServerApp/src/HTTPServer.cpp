...
bool HTTPServer::start()
{
    ...
    {
        string  uri(endpoint.first);
        wstring wur(wstring(endpoint.first.begin(), endpoint.first.end()));

        // Try to bind all end points and handlers
        m_listeners[uri] = make_unique<http_listener>(wur);

        m_listeners[uri]->support(methods::POST,[this, &endpoint](const http_request& request)
        {
