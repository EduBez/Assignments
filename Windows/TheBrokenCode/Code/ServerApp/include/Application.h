...
public:
    ...
    explicit Application(const char* config)
        ...
        , m_server(m_appConfig.get("server"))
    {
        ...
        m_server.registerEndPoint(m_appConfig.get("server")["auth"],
            [this](const web::json::value& data)->ServerApp::SERVER_REPONSE
            {
                return this->handleAuthentication(data);
            });

        m_server.registerEndPoint(m_appConfig.get("server")["record"],
            [this](const web::json::value& data)->ServerApp::SERVER_REPONSE
            {
